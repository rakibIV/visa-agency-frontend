...
export default function RefundPrintView({ applicant, refund, companyInfo }) {
  ...
  const exchangeRate = sampleEur > 0 ? (sampleAmount / sampleEur).toFixed(2) : '140.69';

  const bdtAmount = Number(refund.refund_amount) || 0;
  const eurAmount = (bdtAmount / exchangeRate);
  
  const amountInWordsBDT = numberToWords(Math.floor(bdtAmount)) + ' Taka Only';
          ...
          <div className="flex-1 w-full relative z-20 flex flex-col">
            ...
            <div className="text-center mt-2 mb-4">
              ...
              <div className="flex items-center justify-center gap-4 mt-2">
                ...
                <p className="text-xs font-semibold italic text-slate-800">(Refund Receipt)</p>
                <div className="h-px bg-slate-400 flex-1 max-w-[100px]"></div>
              </div>
            ...
            <div className="flex flex-row gap-6 mb-4">
              ...
              <div className="flex-1 space-y-3 text-[11px] font-semibold text-slate-800 pt-2">
                ...
                <div className="flex"><span className="w-6 text-blue-900">✈️</span><span className="w-28 text-slate-600">Receipt No.</span><span className="mx-2">:</span><span className="uppercase flex-1 border-b border-slate-300 pb-0.5 font-mono">{refund.receipt_number || `REF-${refund.id}`}</span></div>
                <div className="flex"><span className="w-6 text-blue-900">🗓️</span><span className="w-28 text-slate-600">Refund Issue Date</span><span className="mx-2">:</span><span className="uppercase flex-1 border-b border-slate-300 pb-0.5">{new Date(refund.created_at).toLocaleDateString('en-GB')}</span></div>
              </div>
              ...
              <div className="w-[280px] flex flex-col gap-4">
                ...
                <div className="border border-slate-300 rounded-xl flex flex-col text-center overflow-hidden">
                  <div className="p-2 bg-slate-50 text-[9px] font-bold text-slate-600 uppercase border-b border-slate-300">
                    Net Refund Amount Received
                  </div>
                  <div className="p-3 flex-1 flex flex-col justify-center items-center">
                    <span className="font-black text-blue-900 text-lg tracking-wider">BDT {bdtAmount.toLocaleString()}</span>
                    <span className="text-[9px] text-slate-600 mt-1 italic font-serif leading-tight">({amountInWordsBDT})</span>
                  </div>
                  <div className="p-2 bg-orange-50 border-t border-orange-200">
                     <span className="text-[10px] font-bold text-orange-800">EUR Equivalent: €{eurAmount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="border border-blue-900 rounded-xl overflow-hidden mb-4">
               ...
               <div className="p-5 bg-white space-y-4 text-[11px] font-semibold text-slate-800">
                 <div className="flex"><span className="w-8 text-blue-900">💵</span><span className="w-40">Total Amount Paid</span><span className="mx-2">:</span><span className="font-bold">BDT {totalPaid.toLocaleString()}</span></div>
                 <div className="flex"><span className="w-8 text-red-600">📉</span><span className="w-40 text-red-600">Non-Refundable Deductions</span><span className="mx-2">:</span><span className="font-bold text-red-600">- BDT {nonRefundable.toLocaleString()}</span></div>
                 <div className="flex border-t border-slate-200 pt-3"><span className="w-8 text-green-600">💰</span><span className="w-40 text-green-700">Net Refund Amount</span><span className="mx-2">:</span><span className="font-bold text-green-700">BDT {bdtAmount.toLocaleString()}</span></div>
                 <div className="flex mt-2"><span className="w-8 text-blue-900">🏦</span><span className="w-40">Refund Method</span><span className="mx-2">:</span><span className="uppercase">{refundMethodDisplay}</span></div>
                 {refund.bank_detail_snapshot && refund.bank_detail_snapshot.account_number_or_iban && (
                   <div className="flex"><span className="w-8 text-blue-900">🔢</span><span className="w-40">Account No.</span><span className="mx-2">:</span><span className="font-mono">{refund.bank_detail_snapshot.account_number_or_iban}</span></div>
                 )}
               </div>
            </div>
